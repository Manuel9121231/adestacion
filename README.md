# Sistema de Gestión de Máquinas - Guía Rápida

Aplicación web para gestión de incidencias en máquinas (impresoras, impresoras 3D, etc.) usando **Supabase** como base de datos en la nube.
...
| Rol | URL | Descripción |
|-----|-----|-------------|
| Usuario | `/index.html` | Portal de inicio y login |
| Admin | `/dashboard.html` | Panel de administración |
| Operario | `/operario.html` | Registro de incidencias vía QR |
| Estado | `/estado.html` | Vista pública de máquinas |
| TV | `/estado-tv.html` | Vista para pantallas grandes |
...
```
├── index.html              # Portal principal (login/registro)
├── dashboard.html        # Panel de administración
├── operario.html         # Interfaz de operario (QR)
├── estado.html           # Estado público de máquinas
├── estado-tv.html      # Vista para pantallas/TV
├── registro.html       # Crear cuenta de usuario
├── reset-password.html # Recuperar contraseña
├── auth-callback.html  # Callback de autenticación
│
├── server.js             # Servidor Express (solo archivos estáticos + API QR)
├── package.json          # Dependencias
│
├── js/
│   ├── supabase-config.js    # Configuración de Supabase
│   ├── admin.js              # Lógica del panel admin
│   ├── dashboard-ui.js       # UI del dashboard
│   ├── checklist.js          # Checklist de operarios
...
├── database_schema.sql   # Esquema de base de datos (Supabase)
...
La aplicación usa Supabase (PostgreSQL en la nube). La configuración está en:
...
| Tabla | Descripción |
|-------|-------------|
| `salas` | Ubicaciones físicas (ej: "Sala Principal") |
| `equipos` | Máquinas/impresoras con sus características |
| `perfiles` | Usuarios del sistema (auth.users extendido) |
| `registros` | Incidencias reportadas |
| `seguimientos` | Notas de seguimiento a incidencias |
...
|----------|-------------|
| `GET /api/info` | Info del servidor (IP, puerto, URL) - usado para QRs |
