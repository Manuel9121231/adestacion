...
async function detectarServidor() {
  try {
    ...
    console.warn("?? No se pudo obtener IP local del servidor, usando origin actual.");
  }
}
      ...
      <div class="ticket-card ${statusClass} fade-in" onclick="verDetalleSesion('${r.id}')">
        ...
        <div class="ticket-content">
          <div class="ticket-header">
            <div class="ticket-machine-info">
              <span class="machine-name">${r.maquina}</span>
              <span class="sala-name">📍 ${r.sala}</span>
            </div>
            <div class="status-indicator ${statusClass}">${statusText}</div>
          </div>
          
          <div class="ticket-body">
            <p class="ticket-description">${truncate(r.observaciones || 'Sin descripción detallada', 100)}</p>
          </div>

          <div class="ticket-footer">
            <div class="ticket-meta">
              <span class="meta-item">👤 ${r.operario}</span>
              <span class="meta-item">📅 ${formatFechaHora(r.completado_en)}</span>
            </div>
...
function renderUltimosMantenimientos(registros) {
  ...
  tbody.innerHTML = registros.map(r => {
    ...
    const icon = isIncidencia ? (resuelta ? '?' : '??') : '???';
    ...
    return `
      <div class="maquina-card fade-in" 
        ...
        <div class="maquina-actions">
           <button class="btn btn-outline btn-sm" onclick="event.stopPropagation(); editarMaquina('${m.id}')">??</button>
           <button class="btn btn-outline btn-sm" style="color:var(--danger);border-color:var(--danger)" onclick="event.stopPropagation(); eliminarMaquina('${m.id}')">???</button>
        </div>
      </div>
    `;
  }
...
async function toggleResolucionIncidencia(id, nuevoEstado) {
  ...
  if (res.ok) {
    ...
    showFeedback('Error de estado', 'No se pudo actualizar el estado de la incidencia: ' + res.error, 'âŒ');
  }
}
    ...
    <div class="detail-modern">
      ...
      <div class="detail-hero" style="background: ${isInc ? 'rgba(239, 68, 68, 0.08)' : 'rgba(16, 185, 129, 0.08)'}; border-left: 4px solid ${isInc ? 'var(--danger)' : 'var(--success)'}">
        <div class="detail-hero-content">
          <div class="hero-icon">${isInc ? '🚨' : 'ðŸ”§'}</div>
          <div>
      ...
      <div class="detail-grid">
        ...
        <div class="detail-info-card">
          <div class="info-label">ðŸ‘· Responsable del Reporte</div>
            ...
            <div class="detail-gallery">
              ${sesion.fotos.map(f => `
                <div class="gallery-item" onclick="window.open('${f}')">
                  ...
                  <div class="gallery-overlay"><span class="icon">ðŸ”</span></div>
                </div>
              `).join('')}
            </div>
          ` : `
        ...
        timeline.innerHTML = notas.map(n => `
          <div class="timeline-item">
            <div class="timeline-meta">
              ...
              <button class="btn-delete-note" onclick="eliminarNota(${n.id})" title="Eliminar nota">?</button>
            </div>
...
async function eliminarNota(idNota) {
  ...
  const res = await apiFetch(`/api/incidencia/nota/${idNota}`, { method: 'DELETE' });
  if (res.ok) {
    showFeedback('Nota eliminada', 'La nota de seguimiento ha sido eliminada.', '?');
    // Recargar el detalle
    const currentId = window.currentIncidenciaId;
    if (currentId) verDetalleSesion(currentId);
  } else {
    showFeedback('Error al eliminar', res.error, '?');
  }
}
      ...
      <tr>
        ...
        <td data-label="Registro" style="font-size:11px">${fecha}</td>
            <td data-label="Acciones">
              ${esSuperadmin ? `
                <div style="display:flex;gap:6px;flex-wrap:wrap;overflow-x:hidden">
