...
$old = "    // ─── Reenviar código ───`r`n`r`n      btn.disabled = false;`r`n      btn.innerHTML = '🔑 Iniciar Sesión';`r`n    }"
$new = @"
    // ─── Reenviar código ───
...
"@
# We need to escape backticks in the string if we use double quotes, but here I used single quotes for $old and here-string for $new.
# Note: The here-string in PS handles newlines.
